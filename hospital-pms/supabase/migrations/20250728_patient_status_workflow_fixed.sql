...
CREATE INDEX IF NOT EXISTS idx_patient_status_history_patient_id ON public.patient_status_history(patient_id);
CREATE INDEX IF NOT EXISTS idx_patient_status_history_changed_by ON public.patient_status_history(changed_by);
CREATE INDEX IF NOT EXISTS idx_patient_status_history_changed_at ON public.patient_status_history(changed_at DESC);
...
DROP POLICY IF EXISTS "Users can view patient status history based on patient access" ON public.patient_status_history;
DROP POLICY IF EXISTS "Users can add patient status history with permissions" ON public.patient_status_history;
...
-- Function to validate status transitions (optional, mainly handled in app)
...
DROP TRIGGER IF EXISTS validate_patient_status_before_update ON public.patients;
...
-- Drop existing trigger if exists
DROP TRIGGER IF EXISTS auto_create_patient_status_history ON public.patients;
